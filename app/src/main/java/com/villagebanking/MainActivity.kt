        ...
        tvForgotPassword.setOnClickListener {
            ...
            if(cursor.moveToFirst()){
               val pinHint = cursor.getString(cursor.getColumnIndex(DBHandler.ACCOUNT_HOLDERS_PIN_HINT_COL))
                ...
                Toast.makeText(this, "PIN not set. Click LOGIN", Toast.LENGTH_SHORT).show()
            }
            cursor.close()
            db.close()
        }
    ...
    private fun firstTimeUse(){
        ...
        if (cursor.count == 0){
            val firstTimeAlert = AlertDialog.Builder(this)
                    .setTitle("Welcome to - Village Banking by Innovend General Dealers©")
                    .setMessage("First time use does not require a PIN.\n\nPress LOGIN to begin")
